#!/usr/bin/env python3
"""
Lab 2.2 – Gripper sequence
Order: 1 -> 2 -> 4 -> 3 (forward), then back 3 -> 4 -> 2 -> 1 (return).
NOTE: Uses the same air line → air ON = open, air OFF = close.
"""
...
def main():
    ...
    ap.add_argument("--close_dwell", type=float, default=0.6)
    ap.add_argument("--open_dwell", type=float, default=0.25)
    ...
    def pick_then_lift(above, at):
        mov(above); mov(at)
        dev.suck(False)   # OFF = close gripper
        time.sleep(args.close_dwell)
        mov(above)

    def place_then_release(above, at):
        mov(above); mov(at)
        time.sleep(args.hold)
        dev.suck(True)    # ON = open gripper
        time.sleep(args.open_dwell)
        mov(above)
